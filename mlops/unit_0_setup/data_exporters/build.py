...
def test_training_set(
    ...
    *args,
    ...
    assert (
        X_train.shape[1] == 27
    ), f'Training set for training model should have 27 features, but has {X_train.shape[1]}'
    assert (
...
def test_validation_set(
    ...
    *args,
    ...
    assert (
        X_val.shape[0] == 3316216
    ), f'Training set for validation should have 51492 examples, but has {X_val.shape[0]}'
    assert (
        X_val.shape[1] == 3316142
    ), f'Training set for validation should have 3316142 features, but has {X_val.shape[1]}'
    assert (
